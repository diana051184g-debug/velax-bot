...
CACHE_FILE = "users_cache.json"  # Файл для вечного хранения ID людей
...
# Загрузка кэша пользователей (Ник -> ID)
def load_user_cache():
...
# 2. УПРАВЛЕНИЕ ИЗ ЛИЧКИ БОТА (БАН/МУТ/РАЗБАН ПО ВСЕМ ЧАТАМ)
...
def private_admin_commands(message):
    ...
    # Ищем упоминание юзера в тексте
    ...
    # Проверяем, есть ли этот человек в нашей базе данных
    if username not in USER_ID_CACHE:
        bot.reply_to(message, f"❌ Пользователь @{username} не найден в кэше бота!\nОн должен написать хотя бы одно текстовое сообщение в группу «Чатикс», чтобы бот запомнил его ID [💡].")
        return
        
    target_id = USER_ID_CACHE[username]
    now = datetime.now().strftime('%d.%m.%Y %H:%M:%S')

    # ОБРАБОТКА /mute В ЛИЧКЕ
    if command == '/mute':
        ...
        except Exception: bot.reply_to(message, "❌ Ошибка! Проверь, дали ли боту права администратора в чате.")

    # ОБРАБОТКА /ban В ЛИЧКЕ (НАВЕЧНО ИЛИ НА ВРЕМЯ)
    elif command == '/ban':
        ...
        except Exception: bot.reply_to(message, "❌ Ошибка бана. Возможно, его нет в чате или у бота нет прав.")

    # ОБРАБОТКА /unmute В ЛИЧКЕ
    elif command == '/unmute':
        try:
            ...
            bot.reply_to(message, f"😇 **Размут выполнен!** Пользователь @{username} снова может писать во всех группах.")
        except Exception: bot.reply_to(message, "❌ Не удалось снять мут.")

    # ОБРАБОТКА /unban В ЛИЧКЕ
    elif command == '/unban':
        try:
            bot.unban_chat_member(CHAT_ID, target_id, only_if_banned=True)
            bot.reply_to(message, f"🔓 **Разбан выполнен!** Пользователь @{username} успешно удален из черного списка.")
        except Exception: bot.reply_to(message, "❌ Не удалось разбанить.")

# 3. АВТО-МОДЕРАЦИЯ ЧАТА И КЭШИРОВАНИЕ ДАННЫХ
@bot.message_handler(func=lambda msg: msg.chat.id == CHAT_ID)
def moderate_chatix(message):
    user_id = message.from_user.id
    
    # Железно записываем каждого написавшего в кэш
    if message.from_user.username:
        ...
        if USER_ID_CACHE.get(username_lower) != user_id:
            ...
            save_user_cache()  # Сохраняем базу на сервер
...
# 4. НАСТРОЙКА ИЗ ЛИЧКИ И РЕПОРТЫ
user_state = {}
